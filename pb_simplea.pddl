(define (problem pb_simplea)
  ...
  (:init
    ...
    ;; adyacencias bidireccionales (horizontal) - sin paredes
    ...
    ;; adyacencias bidireccionales (vertical) - sin paredes
    ...
    ;; estantería E1: Posición (2,2) 22, pkg2 bottom, pkg1 top
    (on pkg2 ground)
    (on pkg1 pkg2)
    (on-shelf pkg2 e1)
    (on-shelf pkg1 e1)
    (top pkg1)
    ;; estantería E2: (2,5) 25, pkg3
    (on pkg3 ground)
    (on-shelf pkg3 e2)
    (top pkg3)

    ;; zonas importantes
    (at e1 l22) ; Estantería E1
    (at e2 l25) ; Estantería E2
    (at d l44) ; (4,4) Dispensador
  )
